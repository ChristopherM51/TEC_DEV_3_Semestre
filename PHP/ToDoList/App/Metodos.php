...
// Função para excluir uma tarefa pela tarefa
if (isset($_GET['delete'])) {
    $tarefa = $_GET['delete'];
    $queryDelete = "DELETE FROM TODOLIST WHERE tarefa = ?";
    $stmtDelete = $conexao->prepare($queryDelete);
    $stmtDelete->bind_param('tarefa', $tarefa);
    $stmtDelete->execute();
    $stmtDelete->close();

    // Redirecionar de volta para a página de lista após a exclusão
    header('Location: FrontToDoList.php');
    exit();
}
